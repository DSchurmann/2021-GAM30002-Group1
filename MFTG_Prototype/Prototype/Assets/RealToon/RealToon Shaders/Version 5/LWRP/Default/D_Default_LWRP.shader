...
Shader "Lightweight Render Pipeline/RealToon/Version 5/Default/Default"
{
    Properties
    {
		...
		[Toggle] _SPECMODE ("Specular Mode", Float ) = 0
		...
		//_PointLightSVD ("Point Light Shadow Visibility Distance", float ) = 0 //Might Remove
    ...
    SubShader
    {
		...
		Pass {
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
				...
				float node_6587 = 0.0;
				half3 RTD_OL_LAOC_OO = lerp( lerp(_OutlineColor.rgb,_OutlineColor.rgb * _MainTex_var.rgb, _MixMainTexToOutline) , lerp(float3(node_6587,node_6587,node_6587), lerp(_OutlineColor.rgb,_OutlineColor.rgb * _MainTex_var.rgb, _MixMainTexToOutline) ,lightColor.rgb), _LightAffectOutlineColor );
				//
        ...
        Pass
        {
            ...
            half4 LitPassFragment(Varyings input, float facing : VFACE) : SV_Target
            {
				...
				half node_5573 = 1.0;
					...
					half RTD_SS_VCGCSSS_OO = lerp( _SelfShadowThreshold, (_SelfShadowThreshold*(1.0 - input.vertexColor.g)), _VertexColorGreenControlSelfShadowThreshold );
					half RTD_SS_SST = smoothstep( RTD_SS_SSH_Sil, 1.0, (RTD_NDOTL * lerp(7, RTD_SS_VCGCSSS_OO ,_SelfShadowThreshold)) );
					half RTD_SS_SSABLSS_OO = lerp( RTD_SS_SST, lerp(RTD_SS_SST,node_5573, (1.0 - _AdditionalShadowStrength[i]) ), _SelfShadowAffectedByLightShadowStrength );
					half RTD_SS_ON = lerp(node_5573,(RTD_SS_SSABLSS_OO*attenuation),_SelfShadowRealtimeShadowIntensity);
				...
				half3 RTD_CA_OFF_OTHERS =  ((RTD_MCIALO*RTD_SL*RTD_STIAL)+RTD_RL);
					...
					half3 RTD_CA = RTD_CA_OFF_OTHERS * lightfos;
            
				#endif

				float3 add_light_output = RTD_CA;
        ...
        Pass
        {
			...
			//uniform half _PointLightSVD; //5.0.4 //Temporarily Removed
    ...
    CustomEditor "RealToonShaderGUI_LWRP_SRP"
}